...
export class HomePage {
  ...
  constructor(public navCtrl: NavController,
    ...
    docRef.get()
      ...
      .catch((e) => this.MostrarAlert("Ups...", "No podemos establecer conexción. Asegurate de estar conectado.", this.OcultarAlert, true));
  }
        ...
        if (status.authorized) {
          ...
          this.scanSub = this.qrScanner.scan().subscribe((text: string) => {
            ...
            this.VerificarQR(text)
              ...
              .catch((error) => this.MostrarAlert("Ups...", `${error}`, this.InicializarLectorQR, true));
          });
  ...
  presentConfirm() {
    let alert = this.alertCtrl.create({
      title: 'Cerrar sesión.',
      message: '¿Estás seguro/a de que deseas cerrar sesión?',
      buttons: [
        {
          text: 'Cancelar',
          role: 'cancel',
  ...
  ChequearSiElCodigoFueCargado(codigo: string, codigosCargados) {
    ...
    for (let item of codigosCargados) {
      ...
      if (item == codigo) {
        throw "Parece que este código ya lo has cargado previamente, intenta de nuevo."
      }
    }
  }
}